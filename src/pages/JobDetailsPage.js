...
const Tag = ({ text, primary }) => {
  ...
  return primary ? (
    ...
    <span className="inline-flex items-center px-2.5 py-1 rounded-md text-xs font-medium bg-[var(--glass-hover)] text-ink-muted border border-[rgb(var(--line)/0.15)]">
      {text}
    </span>
  );
};
...
const ExigenceBadge = ({ code }) => {
  ...
  return (
    <span className={`ml-1.5 text-xs px-1.5 py-0.5 rounded font-medium ${
      code === 'E' ? 'bg-red-50 text-red-600' : 'bg-[var(--glass-hover)] text-ink-muted'
    }`}>
...
const InfoRow = ({ icon, label, children }) => (
  <div className="py-4 border-b border-[rgb(var(--line)/0.1)] last:border-0 sm:flex sm:gap-4">
    <div className="flex items-center gap-1.5 mb-1.5 sm:mb-0 sm:w-36 sm:shrink-0 sm:items-start sm:pt-0.5">
      ...
      <span className="text-xs font-semibold uppercase tracking-wide text-ink-faint sm:text-sm sm:font-medium sm:normal-case sm:tracking-normal sm:text-ink-muted">{label}</span>
    </div>
...
const SubLabel = ({ children }) => (
  <p className="text-xs font-semibold text-ink-faint uppercase tracking-wide mb-1.5">{children}</p>
);
...
const JobDetailsPage = () => {
  ...
  if (isLoading) {
    return (
      <div className="max-w-4xl mx-auto py-6 px-4 sm:px-6 lg:px-8 text-center">
        <div className="py-20">
          <svg className="animate-spin h-10 w-10 text-accent mx-auto" fill="none" viewBox="0 0 24 24">
            <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" />
            <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z" />
          </svg>
          <p className="mt-4 text-ink-muted">Chargement de l'offre…</p>
        </div>
      </div>
    );
  }
  ...
  if (isError || !job) {
    return (
      <div className="max-w-4xl mx-auto py-6 px-4 sm:px-6 lg:px-8">
        ...
        <button onClick={handleBack} className="inline-flex items-center gap-2 px-4 py-2 border border-[rgb(var(--line)/0.15)] rounded-lg text-sm font-medium text-ink-muted glass-card hover:bg-[var(--glass-hover)] transition-colors">
  ...
  return (
    <div className="max-w-4xl mx-auto py-6 px-4 sm:px-6 lg:px-8">
      ...
      <button
        onClick={handleBack}
        className="mb-5 inline-flex items-center gap-2 text-sm font-medium text-ink-muted hover:text-ink transition-colors"
      >
        <svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
          <path strokeLinecap="round" strokeLinejoin="round" d="M15 19l-7-7 7-7" />
        </svg>
      ...
      <div className="glass-card rounded-xl border border-[rgb(var(--line)/0.15)] shadow-sm overflow-hidden">
        ...
        <div className="p-4 sm:p-6">
          <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
            <div className="flex-1 min-w-0">
              <h1 className="text-xl font-bold text-ink leading-snug">{job.intitule}</h1>

              <div className="mt-2 space-y-1">
                {job.entreprise?.nom && (
                  <div className="inline-flex items-center gap-1.5 text-sm text-ink-muted font-medium">
                    <svg className="h-4 w-4 text-ink-faint shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
              ...
              <button
                ...
                className={`inline-flex items-center gap-2 px-4 py-2 rounded-lg border text-sm font-medium transition-colors ${
                  isSaved
                    ...
                    : 'glass-card text-ink-muted border-[rgb(var(--line)/0.15)] hover:bg-[var(--glass-hover)]'
                }`}
              >
        ...
        {/* Infos clés */}
        <div className="px-4 sm:px-6 pb-2 border-t border-[rgb(var(--line)/0.1)]">
          <InfoRow
            label="Salaire"
            icon={
              <svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
          ...
          >
            <span className={`font-medium ${hasSalary ? 'text-green-700' : 'text-ink-faint'}`}>
              {salary}
            </span>
            {job.salaire?.libelle && hasSalary && (
              <p className="mt-0.5 text-xs text-ink-faint">{job.salaire.libelle}</p>
            )}
            {job.salaire?.commentaire && (
              <p className="mt-0.5 text-xs text-ink-muted italic">{job.salaire.commentaire}</p>
            )}
            {(job.salaire?.complement1 || job.salaire?.complement2) && (
              <ul className="mt-1 space-y-0.5 text-xs text-ink-muted list-disc list-inside">
          ...
          >
            ...
            {job.natureContrat && <span className="text-ink-faint"> · {job.natureContrat}</span>}
          </InfoRow>
          ...
          {(job.conditionExercice || job.complementExercice || job.contexteTravail?.horaires || job.contexteTravail?.conditionsExercice) && (
            ...
            >
              <ul className="space-y-0.5">
                ...
                {job.complementExercice && <li className="text-ink-muted">{job.complementExercice}</li>}
              </ul>
            </InfoRow>
          )}
        </div>
        ...
        {/* Description */}
        <div className="px-4 sm:px-6 py-5 border-t border-[rgb(var(--line)/0.1)]">
            ...
            <div className="space-y-5">
              ...
              {job.experienceCommentaire && (
                <div>
                  ...
                  <p className="text-sm text-ink-muted">{job.experienceCommentaire}</p>
                </div>
              )}
                      ...
                      <li key={i} className="text-sm text-ink-muted">
                        <div className="flex items-baseline gap-1.5 flex-wrap">
                          ...
                          {f.domaineLibelle && f.niveauLibelle && (
                            <span className="text-ink-faint">— {f.domaineLibelle}</span>
                          )}
                          <ExigenceBadge code={f.exigence} />
                        </div>
                        {f.commentaire && (
                          <p className="mt-0.5 text-xs text-ink-muted">{f.commentaire}</p>
                        )}
                      </li>
                    ))}
                  </ul>
                      ...
                      <span
                        ...
                        className={`inline-flex items-center gap-1 px-2.5 py-1 rounded-md text-xs font-medium border ${
                          c.exigence === 'E'
                            ...
                            : 'bg-[var(--glass-hover)] text-ink-muted border-[rgb(var(--line)/0.15)]'
                        }`}
                      >
                        {c.libelle}
                        {c.exigence === 'E' && (
                  ...
                  <div className="space-y-2">
                    {job.qualitesProfessionnelles.map((q, i) => (
                      <div key={i} className="flex items-start gap-2">
                        <svg className="mt-0.5 h-3.5 w-3.5 text-accent/50 shrink-0" fill="currentColor" viewBox="0 0 20 20">
                          <path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clipRule="evenodd" />
                        </svg>
                        <div>
                          <span className="text-sm font-medium text-ink">{q.libelle}</span>
                          {q.description && (
                            <p className="text-xs text-ink-muted mt-0.5">{q.description}</p>
                          )}
                        </div>
                      </div>
                    ))}
              ...
              {job.outilsBureautiques && (
                <div>
                  ...
                  <p className="text-sm text-ink-muted">{job.outilsBureautiques}</p>
                </div>
              )}
                  ...
                  {hasLangues && (
                    <div>
                      ...
                      <ul className="space-y-1">
                        {job.langues.map((l, i) => (
                          <li key={i} className="flex items-center text-sm text-ink-muted">
                            {l.libelle}
          ...
          <div className="px-4 sm:px-6 py-5 border-t border-[rgb(var(--line)/0.1)]">
            ...
            <div className="space-y-3">
              {job.entreprise.description && (
                <p className="text-sm text-ink-muted leading-relaxed">{job.entreprise.description}</p>
              )}
              {job.entreprise.url && (
                <a
        ...
        {hasContactBlock && (
          <div className="px-4 sm:px-6 py-5 border-t border-[rgb(var(--line)/0.1)] bg-[var(--glass-hover)]/50 rounded-b-xl">
            ...
            <div className="space-y-2">
              ...
              {job.contact.telephone && (
                <div className="flex items-center gap-2 text-sm">
                  ...
                  <a href={`tel:${job.contact.telephone}`} className="text-accent hover:underline">
                    {job.contact.telephone}
                  </a>
                </div>
              )}
              {job.contact.courriel && !courrielUrl && (
                <div className="flex items-center gap-2 text-sm">
                  ...
                  <a href={`mailto:${job.contact.courriel}`} className="text-accent hover:underline">
                    {job.contact.courriel}
                  </a>
                </div>
              )}
              {job.contact.commentaire && (
                <div className="mt-3 p-3 glass-card border border-[rgb(var(--line)/0.15)] rounded-lg text-sm text-ink-muted whitespace-pre-wrap">
